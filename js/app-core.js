...
class App {
  constructor() {
    ...
    this._lastAccuracy = null;        // 最近一次定位精度（米）
    this._theme = 'dark';             // 主题：dark | light
    this._accent = 'cyan';            // 主色方案
  ...
  init() {
    ...
    // 恢复轨迹平滑偏好
    try {
      const pref = localStorage.getItem('tujie_trail_smooth') || localStorage.getItem('circlemap_trail_smooth');
    ...
    // 初始化会话管理
    this._initSessions().catch(console.warn);
  ...
  _setupUI() {
    ...
    document.getElementById('batch-export-btn').addEventListener('click', () => this._exportBatchSessions());
    document.getElementById('select-all-btn').addEventListener('click', () => this._selectAllSessions());
    document.getElementById('sessions-clear-all-btn').addEventListener('click', () => this._clearAllSessions());

    // 回放控制按钮
    document.getElementById('replay-play-btn').addEventListener('click', () => {
      if (this._replayAnimId) this._pauseReplay();
      else this._resumeReplay();
      const btn = document.getElementById('replay-play-btn');
      if (btn) btn.textContent = this._replayAnimId ? '⏸' : '▶';
    });
    document.getElementById('replay-close-btn').addEventListener('click', () => this._stopReplay());
    document.querySelectorAll('.speed-btn').forEach(btn => {
      btn.addEventListener('click', () => this._setReplaySpeed(parseFloat(btn.dataset.speed)));
    });
    const replayProg = document.getElementById('replay-progress');
    if (replayProg) {
      replayProg.addEventListener('input', () => {
        const all = [...this._sessions, ...this._recordingSessions];
        const session = all.find(s => s.id === this._replayingId);
        if (!session) return;
        const progress = replayProg.value / 1000;
        const idx = Math.floor(progress * (session.positions.length - 1));
        const pos = session.positions[idx];
        this.mapManager.advanceReplayTo(pos, idx);
        this._replayElapsed = progress * (session.durationMs || 1);
        this._updateReplayUI(session, progress, pos);
      });
    }
...
/**
   ...
   _toggleTrailRecording() {
    if (this.trail.isRecording) {
      ...
      if (pointCount === 0) {
        ...
        const name = this._autoNameSession();
        this.sessionManager.save(this.trail, name).then(() => {
          this._loadSessions();
        }).catch(console.warn);
        Storage.saveTrail(this.trail);
      }
      Toast.show(pointCount === 0 ? ' 未记录到轨迹数据' : ' 轨迹记录已保存');
    } else {
       this.trail.start();
       this.mapManager.clearTrail();
       Toast.show(' 轨迹记录已开始');
  ...
  _autoNameSession() {
    const now = new Date();
    const today = now.toLocaleDateString('zh-CN', { month: '2-digit', day: '2-digit' });
    const time = now.toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit', hour12: false });
    return `轨迹 ${today} ${time}`;
  }
  ...
  _toggleTrailSmoothing() {
    ...
    // 保存偏好
    try {
      localStorage.setItem('tujie_trail_smooth', this._trailSmoothing ? '1' : '0');
    } catch (e) { /* 静默 */ }
    // 刷新轨迹渲染
  ...
  async _exportReport() {
    ...
    try {
      ...
      const filename = `tujie-activity-${dateStr}.png`;
  ...
  async _exportReportForSessions(sessions) {
    if (sessions.length === 1) {
      await this._exportReport(sessions[0].positions, sessions[0].name, sessions[0].createdAt);
      return;
    }
    const isDark = this._theme === 'dark';
    ...
    for (let i = 0; i < sessions.length; i++) {
      await this._renderSessionSegmentToCanvas(ctx, sessions[i], i * SEG_H, isDark, W, H, i === sessions.length - 1, S, sessions.length, i);
      if (i < sessions.length - 1) {
        const sepY = (i + 1) * SEG_H;
        ctx.strokeStyle = isDark ? 'rgba(255,255,255,0.15)' : 'rgba(0,0,0,0.12)';
        ctx.lineWidth = 1 * S;
        ctx.setLineDash([4 * S, 4 * S]);
        ctx.beginPath();
        ctx.moveTo(24 * S, sepY);
        ctx.lineTo(W - 24 * S, sepY);
        ctx.stroke();
        ctx.setLineDash([]);
      }
    }
    const dateStr = new Date().toISOString().slice(0, 19).replace(/[T:]/g, '-');
    const filename = `tujie-batch-${dateStr}.png`;
    canvas.toBlob(async (blob) => {
      if (!blob) { Toast.show(' 导出失败'); return; }
      if (typeof Capacitor !== 'undefined' && Capacitor.isNativePlatform()) {
        try {
          const reader = new FileReader();
          const base64 = await new Promise(r => { reader.onloadend = () => r(reader.result.split(',')[1]); reader.readAsDataURL(blob); });
          const result = await Capacitor.Plugins.Filesystem.writeFile({ path: filename, data: base64, directory: 'CACHE' });
          await Capacitor.Plugins.Share.share({ title: '途刻 活动报告', text: `途刻 — ${sessions.length} 条轨迹活动报告`, url: result.uri });
          Toast.show(' 报告已分享');
        } catch (e) { console.error('[Export] 分享失败:', e); Toast.show(' 分享失败'); }
      } else {
        const url = URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.download = filename;
  ...
  async _renderSessionSegmentToCanvas(ctx, session, yStart, isDark, W, H, isLast, S, totalSessions, index) {
    const pos = session.positions;
    const totalDist = session.distance || 0;
    const firstTime = pos[0].time || null;
    const lastTime = pos[pos.length - 1].time || null;
    const durationMs = lastTime > firstTime ? lastTime - firstTime : 0;
    let maxSpeed = 0;
    for (const p of pos) { if (p.speed != null && p.speed > maxSpeed) maxSpeed = p.speed; }
    const avgSpeed = durationMs > 0 ? totalDist / (durationMs / 1000) : 0;
    const fmtDate = (ts) => { if (!ts) return '--'; const d = new Date(ts); const pad = n => String(n).padStart(2,'0'); return `${pad(d.getMonth()+1)}-${pad(d.getDate())} ${pad(d.getHours())}:${pad(d.getMinutes())}`; };
    const fmtDuration = (ms) => { if (!ms || ms <= 0) return '--'; const s = Math.round(ms / 1000); const m = Math.floor(s / 60); const sec = s % 60; return `${m}:${String(sec).padStart(2,'0')}`; };
    // 段标题
    ctx.fillStyle = isDark ? '#16213e' : '#ffffff';
    ctx.fillRect(0, yStart, W, 48 * S);
    ctx.fillStyle = isDark ? '#00D4AA' : '#0ea5e9';
    ctx.font = `${20 * S}px "HarmonyOS Sans", sans-serif`;
    const segLabel = (totalSessions > 1 && index !== undefined) ? `第${index + 1}条 · ` : '';
    ctx.fillText(`${segLabel}${session.name}`, 24 * S, yStart + 30 * S);
    ctx.fillStyle = isDark ? 'rgba(255,255,255,0.5)' : 'rgba(0,0,0,0.5)';
    ctx.font = `${12 * S}px "HarmonyOS Sans", sans-serif`;
    ctx.fillText(`${fmtDate(session.createdAt)}  ${totalDist > 0 ? formatDistance(totalDist) : '0m'}  ${fmtDuration(session.durationMs)}`, 24 * S, yStart + 44 * S);
    // 地图区
    const mapY = yStart + 48 * S;
    const mapH = 320 * S;
    const mapW = W - 48 * S;
    const mapX = 24 * S;
    ctx.fillStyle = isDark ? '#0f3460' : '#dce5f0';
    ...
    for (const p of pos) { if (p.lat < minLat) minLat = p.lat; if (p.lat > maxLat) maxLat = p.lat; if (p.lng < minLng) minLng = p.lng; if (p.lng > maxLng) maxLng = p.lng; }
    const rawLatSpan = maxLat - minLat || 0.001;
    const rawLngSpan = maxLng - minLng || 0.001;
    const padR = Math.max(0.001, Math.max(rawLatSpan, rawLngSpan) * 0.5);
    minLat -= padR; maxLat += padR; minLng -= padR; maxLng += padR;
    const lngSpan = maxLng - minLng || 0.001;
    const latSpan = maxLat - minLat || 0.001;
    const margin = 20 * S;
    const drawW = mapW - margin * 2;
    ...
    const dataH = latSpan;
    const scale = Math.min(drawW / dataW, drawH / dataH);
    const originX = mapX + margin + (drawW - dataW * scale) / 2;
    const originY = mapY + margin + (drawH - dataH * scale) / 2;
    const toX = (lng) => originX + (lng - minLng) * cosLat * scale;
    ...
    const flushBatch = () => {
      ...
      ctx.beginPath();
      ctx.moveTo(batchPath[0].x, batchPath[0].y);
      for (let j = 1; j < batchPath.length; j++) ctx.lineTo(batchPath[j].x, batchPath[j].y);
      ctx.stroke();
    };
    for (let i = 1; i < pos.length; i++) {
      const speed = pos[i].speed != null ? pos[i].speed : 0;
      const key = getSpeedKey(speed);
      if (key !== batchKey) { flushBatch(); batchPath = [{ x: toX(pos[i-1].lng), y: toY(pos[i-1].lat) }]; batchKey = key; }
      batchPath.push({ x: toX(pos[i].lng), y: toY(pos[i].lat) });
    }
    flushBatch();
    // 起点/终点
    if (pos.length >= 2) {
      ctx.fillStyle = '#22c55e';
      ctx.beginPath(); ctx.arc(toX(pos[0].lng), toY(pos[0].lat), 5 * S, 0, Math.PI * 2); ctx.fill();
      ctx.fillStyle = '#ef4444';
      ctx.beginPath(); ctx.arc(toX(pos[pos.length-1].lng), toY(pos[pos.length-1].lat), 5 * S, 0, Math.PI * 2); ctx.fill();
    }
    // 速度图例
    const legendY = mapY + mapH + 4 * S;
    ctx.fillStyle = isDark ? 'rgba(255,255,255,0.6)' : 'rgba(0,0,0,0.5)';
    ctx.font = `${10 * S}px "HarmonyOS Sans", sans-serif`;
    ctx.fillText(' 0-10  10-20  20-60  60-120  120-200  200-350  >350 km/h', mapX + 8 * S, legendY + 14 * S);
    const legendColors = ['#00E5CC','#FFD700','#FF8C00','#FF5E33','#FF3366','#BF40FF','#5E5CE6'];
    let lx = mapX + 8 * S;
    for (const c of legendColors) {
      ctx.fillStyle = c;
      ctx.fillRect(lx, legendY, 10 * S, 10 * S);
      lx += 14 * S;
    }
    // 统计区
    const statsY = legendY + 32 * S;
    ctx.fillStyle = isDark ? '#16213e' : '#ffffff';
    ctx.beginPath();
    ctx.roundRect(mapX, statsY, mapW, 160 * S, 12 * S);
    ctx.fill();
    ...
    const stats = [
      ...
      { label: '总时长', value: fmtDuration(durationMs) },
      { label: '平均速度', value: avgSpeed > 0 ? (avgSpeed * 3.6).toFixed(1) + ' km/h' : '--' },
      { label: '最高速度', value: maxSpeed > 0 ? (maxSpeed * 3.6).toFixed(1) + ' km/h' : '--' },
      { label: '轨迹点数', value: String(pos.length) },
    ];
    ctx.fillStyle = isDark ? 'rgba(255,255,255,0.5)' : 'rgba(0,0,0,0.5)';
    ctx.font = `${12 * S}px "HarmonyOS Sans", sans-serif`;
    ...
    for (let i = 0; i < stats.length; i++) {
      ...
      const sx = mapX + 16 * S + col * colW, sy = statsY + 56 * S + row * 48 * S;
      ctx.fillText(stats[i].label, sx, sy);
      ctx.fillStyle = isDark ? 'rgba(255,255,255,0.9)' : 'rgba(0,0,0,0.9)';
      ctx.font = `${18 * S}px "HarmonyOS Sans", sans-serif`;
      ctx.fillText(stats[i].value, sx, sy + 22 * S);
      ctx.fillStyle = isDark ? 'rgba(255,255,255,0.5)' : 'rgba(0,0,0,0.5)';
      ctx.font = `${12 * S}px "HarmonyOS Sans", sans-serif`;
    }
    if (!isLast) {
      ...
      ctx.font = `${11 * S}px "HarmonyOS Sans", sans-serif`;
      ctx.textAlign = 'right';
      ctx.fillText('途刻 · TraceCraft', W - 24 * S, yStart + SEG_H - 16 * S);
      ctx.textAlign = 'left';
    }
  }
    ...
    },
{
  ...
  async _loadSessions() {
    const { sessions, recordings } = await this.sessionManager.loadAll();
    this._sessions = sessions;
    this._recordingSessions = recordings;
    this._renderSessionList();
  }

  async _toggleFavorite(id, btnEl) {
    try {
      const isFav = await this.sessionManager.toggleFavorite(id);
      if (btnEl) {
        btnEl.classList.toggle('active', isFav);
        btnEl.textContent = isFav ? '★' : '☆';
      }
    } catch (e) {
      if (e.message === 'FAVORITE_LIMIT') Toast.show(' 收藏已达上限（最多5条）');
    }
  }

  async _deleteSession(id) {
    if (!confirm('确定删除该条轨迹？')) return;
  ...
  _editSessionName(id, el) {
    const current = this._sessions.find(s => s.id === id)?.name
      || this._recordingSessions.find(s => s.id === id)?.name;
    if (!current) return;
    ...
    const finish = async () => {
      ...
      await this.sessionManager.updateName(id, name);
      this._renderSessionList();
    };
  ...
  async _startReplay(sessionId) {
    if (this._replayingId === sessionId) { this._stopReplay(); return; }
    const all = [...this._sessions, ...this._recordingSessions];
    const session = all.find(s => s.id === sessionId);
    if (!session || session.positions.length < 2) return;
    this._wasWatchingBeforeReplay = this._isWatching;
    if (this._isWatching) this._stopWatching();
    this._replayingId = sessionId;
    this._replaySpeed = 1;
    this._replayElapsed = 0;
    await this.mapManager.replayTrail(session);
    this._showReplayControl();
  ...
  _playReplay() {
    if (this._replayingId === null) return;
    const all = [...this._sessions, ...this._recordingSessions];
    const session = all.find(s => s.id === this._replayingId);
    if (!session) { this._stopReplay(); return; }
    const totalMs = session.durationMs || 1;
    const speed = this._replaySpeed;
    const now = Date.now();
    const delta = (now - this._replayStartTime) * speed;
    this._replayElapsed += delta;
    this._replayStartTime = now;
    const progress = Math.min(this._replayElapsed / totalMs, 1);
    const idx = Math.floor(progress * (session.positions.length - 1));
    const pos = session.positions[idx];
    this.mapManager.advanceReplayTo(pos, idx);
    this._updateReplayUI(session, progress, pos);
    if (progress < 1) {
      this._replayAnimId = requestAnimationFrame(() => this._playReplay());
    } else {
      this._stopReplay();
    }
  }
  ...
  _resumeReplay() {
    if (this._replayAnimId) return;
    this._replayStartTime = Date.now() - this._replayElapsed / this._replaySpeed;
    this._playReplay();
  }
  ...
  _stopReplay() {
    if (this._replayAnimId) { cancelAnimationFrame(this._replayAnimId); this._replayAnimId = null; }
    this._replayingId = null;
    this.mapManager.stopReplay();
    this._hideReplayControl();
    if (this._wasWatchingBeforeReplay) this._startWatching();
    this._renderSessionList();
  }

  _setReplaySpeed(speed) {
    this._replaySpeed = speed;
    this._replayStartTime = Date.now() - this._replayElapsed / speed;
    document.querySelectorAll('.speed-btn').forEach(b => {
      b.classList.toggle('active', parseFloat(b.dataset.speed) === speed);
    });
  }

  _updateReplayUI(session, progress, pos) {
    const timeEl = document.getElementById('replay-time');
    const distEl = document.getElementById('replay-dist');
    const speedEl = document.getElementById('replay-speed');
    const progEl = document.getElementById('replay-progress');
    if (timeEl) {
      const cur = Math.round(progress * (session.durationMs || 1));
      timeEl.textContent = `${this._formatDuration(cur)} / ${this._formatDuration(session.durationMs)}`;
    }
    if (distEl) {
      let d = 0;
      for (let i = 1; i <= Math.min(pos ? session.positions.indexOf(pos) : 0, session.positions.length - 1); i++) {
        d += calcDistance(session.positions[i-1], session.positions[i]);
      }
      distEl.textContent = formatDistance(d);
    }
    if (speedEl && pos != null && pos.speed != null) speedEl.textContent = (pos.speed * 3.6).toFixed(1) + ' km/h';
    if (progEl) progEl.value = Math.round(progress * 1000);
  }

  _showReplayControl() {
    const el = document.getElementById('replay-control');
  ...
  _toggleSessionSelect(id) {
    if (this._selectedSessionIds.has(id)) {
      ...
      if (this._selectedSessionIds.size >= CONFIG.TRAIL_SESSION_BATCH_EXPORT_MAX) {
        Toast.show(` 最多选择 ${CONFIG.TRAIL_SESSION_BATCH_EXPORT_MAX} 条导出`);
        return;
      }
  ...
  async _exportBatchSessions() {
    if (this._selectedSessionIds.size === 0) return;
    const sessions = [...this._selectedSessionIds]
      .map(id => [...this._sessions, ...this._recordingSessions].find(s => s.id === id))
      .filter(Boolean)
      .sort((a, b) => a.createdAt - b.createdAt);
    await this._exportReportForSessions(sessions);
    this._selectedSessionIds.clear();
    this._renderSessionList();
    this._updateBatchExportBtn();
  }

  async _selectAllSessions() {
    const all = [...this._sessions, ...this._recordingSessions];
    const allSelected = all.length > 0 && all.every(s => this._selectedSessionIds.has(s.id));
    if (allSelected) {
      this._selectedSessionIds.clear();
    } else {
      const limit = Math.min(all.length, CONFIG.TRAIL_SESSION_BATCH_EXPORT_MAX);
      all.slice(0, limit).forEach(s => this._selectedSessionIds.add(s.id));
    }
    this._renderSessionList();
    this._updateBatchExportBtn();
  }
  ...
  async _clearAllSessions() {
    if (!confirm('确定清空全部历史轨迹？收藏的也会被删除。')) return;
    await this.sessionManager.deleteAll();
    this._selectedSessionIds.clear();
    this._renderSessionList();
    this._updateBatchExportBtn();
  }
