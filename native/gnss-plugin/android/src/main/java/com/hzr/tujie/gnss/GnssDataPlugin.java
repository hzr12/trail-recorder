 ...
 * GNSS 原始数据插件�?
 *
 * 桥接 Android LocationManager 的卫星状态和 NMEA 数据�?JavaScript�?
 * 可获取：卫星数、信噪比、星座类型、仰�?方位角、NMEA 原始语句�?
 *
 * 事件:
 *   - "gnssStatus"   : GnssSatelliteInfo[] �?卫星状态更�?(~1s/�?
 *   - "nmeaSentence" : GnssNmeaData        �?每收到一�?NMEA 语句触发
 */
@CapacitorPlugin(
    name = "GnssData",
    permissions = {
...
public class GnssDataPlugin extends Plugin {
    ...
    /**
     * 开始监�?GNSS 原始数据�?
     * 需�?ACCESS_FINE_LOCATION 权限（已�?@CapacitorPlugin 中声明）�?
     *
     * 启动�?
     *   - 卫星状态通过 "gnssStatus" 事件推�?
     *   - NMEA 语句通过 "nmeaSentence" 事件推�?
     */
    @PluginMethod
    public void startGnssListening(PluginCall call) {
        ...
        // ⚠️ 不要�?Capacitor �?hasPermission("location") alias 检�?
        // 原因：浏览器 Geolocation �?Capacitor plugin 的权限别名不同步�?
        // 例如：Geolocation API 已工作（精度 5m）但 GnssDataPlugin �?alias 仍为 not granted
        // 直接�?Android 系统权限检查，拿到的就�?LocationManager 真实可用的状�?
        Context ctx = getContext();
        if (ctx == null) {
        ...
        if (perm != PackageManager.PERMISSION_GRANTED) {
            ...
            // 不直�?reject，让 try/catch 兜底。LocationManager 内部会再次校验权限�?
        } else {
        ...
        try {
            ...
            // 清理可能已成功注册的部分（registerGnssCallback 可能已注�?GnssStatus callback�?
            unregisterGnssCallback();
            unregisterNmeaListener();
    ...
    private void registerGnssCallback() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            ...
            try {
                // 必须�?Handler �?主线�?Looper。原因：
                // 单参�?registerGnssStatusCallback(callback) 要求调用方必须是 Looper 线程�?
                // �?Capacitor @PluginMethod 默认在线程池里跑，不�?Looper�?
                // �?Handler �?callback 固定在主线程触发，回调和 notifyListeners 都在 Looper 上，不会丢事件�?
                ...
                // API 24/25 上某�?ROM �?registerGnssStatusCallback 仍然单参数重载不识别
                // 无参 fallback 一�?
    ...
    private void registerNmeaListener() {
        ...
        try {
            // 同样传主线程 handler �?避免 capacitor 线程池不�?looper 线程导致 callback 永远不触�?
            Handler mainHandler = new Handler(Looper.getMainLooper());
