...
途刻 TraceCraft：一个**纯前端**的轨迹记录/回放 PWA（无构建工具、无框架、无测试套件），核心是腾讯地图 + GPS 追踪。根目录是可直接用静态服务器打开的 web 应用；`native/` 是 Capacitor 封装的 Android 壳（含 GNSS 卫星数据插件）。
...
- **`js/gps.js`**：`GPSManager` + `KalmanFilter`（2D 恒速模型卡尔曼滤波，局部 ENU 米坐标）。封装浏览器 Geolocation，支持单次定位、持续追踪、精度降级/恢复、省电模式，回调通过 `onPositionChange`/`onError` 等注入。
