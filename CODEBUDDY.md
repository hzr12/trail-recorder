...
- **`js/gps-imm.js`**：`ImmFilter`（交互式多模型实时滤波，6 维×3 模型：静止/CV/恒加速，x/y 两轴解耦成 3×3 子问题优化矩阵运算）。**IMU 职责收窄为纯加速度注入校准**：`ImmFilter.feedImu()` 把 1Hz 聚合的 ENU 加速度注入 CA 模型预测（`x⁻=F·x̂+G·a_imu`，`G=[½dt²,dt,0]ᵀ` 只影响位置/速度预测，加速度状态保持模型自持；注入期 CA 模型 Q 缩放 `max(0.3, 1−0.7·trust)`；仅运动学先验，GPS 仍是位置权威）。**注入自适应**：trust 随 IMU 推断速度变化（a·dt）与滤波输出速度变化的方向一致性动态调权（`IMU_TRUST_STEP`、`IMU_ACC_TRUST_MIN/MAX`），低速时向基础值回归；注入幅值按 GPS 速度分级 clamp（`IMU_ACC_CLAMP_LEVELS`：静止收紧防噪声、高速放宽保机动）；并受重力泄漏衰减（`ImuManager.tiltLeakFactor`，姿态误差使约 g·sinε 重力泄漏到水平轴时削减注入）。
- **`js/gps-alt.js`**：海拔滤波链 `AltKalmanFilter`（1D 自适应卡尔曼）+ `AltFilterPipeline`（实时流水线）+ `AltRtsSmoother`（离线 1D RTS）。**IMU 垂直融合**（方向 3）：`AltKalmanFilter.feedAccU()` 把 ENU U 轴线性加速度（`TYPE_LINEAR_ACCELERATION` 已去重力）注入 CA 预测（`x⁻=F·x̂+G·u`，`G=[½dt²,dt]ᵀ`，仅运动学先验），GPS 海拔仍是观测权威（零基准由 GPS 持续校正，绝不做纯积分）；垂直注入按 GPS 速度分级 clamp + `ALT_IMU_TRUST` 缩放，web 无插件时静默跳过。
- **`js/gps-imu.js`**：`ImuManager`（原生 `ImuData` 插件桥接，**仅定位校准**：设备系线性加速度经 rotation 四元数旋转到 ENU 地理系 → **滑窗均值**（近 `IMU_FEED_INTERVAL_MS` 窗口分桶环形缓冲，均值持续更新）→ 一阶低通 → 注入滤波预测。**姿态-加速度时间对齐**（方向 1）：插件下发 `rotationTs`（姿态事件时间戳），旋转时按加速度事件时间戳查最近姿态（超 `IMU_ROT_MAX_DT_MS` 视为不匹配降级）；**三轴输出 E/N/U**（方向 2，U 供海拔 CA 注入）；**U 轴偏置统计**（方向 6，低动态时慢速低通）估计重力泄漏量级 → `tiltLeakFactor` 衰减水平注入）。**明确不做**：GPS 丢失航迹推算（无 `predictOnly`/DR 状态机）、IMU 航向解算（航向由 GPS 权威——NMEA VTG/RMC + 浏览器 `coords.heading`，IMU 不读陀螺仪融合；GPS 航向缺失/低速时由 `_resolveHeadingFallback` 用滤波后相邻点位移差分兜底 + 一阶低通）。IMU 随 watch 生命周期启停（`_startImu`/`_stopImu`），省电模式同步关闭；web 端无插件时静默跳过，纯 GPS 零回归。
