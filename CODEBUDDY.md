...
- **`js/config.js`**：`CONFIG` 常量集中地（地图 key、GPS 参数、速度等级表、存储配置、交互阈值）。**同时定义全局工具函数**：`calcDistance`（Haversine）、`calcBearing`、`bearingToDir`、`formatDistance`、`formatDurationShort`、`ddToDms`、`copyText`。速度等级表 `CONFIG.TRAIL_SPEED_LEVELS` 是着色与分段的**单一数据源**，修改需保持 `map.js` 与 `trail-analysis.js` 同步消费。
...
- **`js/gps.js`**：`GPSManager` + `ImmFilter`（交互式多模型实时滤波，6 维×3 模型：静止/CV/恒加速，x/y 两轴解耦成 3×3 子问题优化矩阵运算）+ `KalmanFilter`（仅服务离线 RTS 平滑，独立实例 `_offlineSmoother`，与实时层彻底解耦）+ `ImuManager`（原生 `ImuData` 插件桥接：四元数姿态旋转到 ENU 地理系、1s 窗口均值聚合/逐点低通、推算高频切换）。封装浏览器 Geolocation，支持单次定位、持续追踪、精度降级/恢复、省电模式，回调通过 `onPositionChange`/`onError` 等注入。**IMU 融合两阶段**：阶段二 `ImmFilter.feedImu()` 把 1Hz 聚合的 ENU 加速度注入 CA 模型预测（`x⁻=F·x̂+G·a_imu`，只做运动学先验，GPS 仍是位置权威）；阶段三 GPS 丢失（accuracy>2000m 且卫星<4）时 `ImmFilter.predictOnly()` 只预测不更新做短时航迹推算，推算状态机由 `_maybeEnterDeadReckoning`/`_advanceDeadReckoning`/`_exitDeadReckoning` 管理；GPS 彻底断信号（watchPosition 回调停止，`_maybeEnterDeadReckoning` 失去执行入口）时由超时看门狗 `_tryEnterDeadReckoningFromTimeout` 用最近一次可信 fix 兜底触发，上限 `IMU_DEAD_RECKON_MAX_MS`=8s（纯积分漂移物理上限），恢复后一次 GPS fix 重锚无缝接回。IMU 随 watch 生命周期启停，省电模式同步关闭。
